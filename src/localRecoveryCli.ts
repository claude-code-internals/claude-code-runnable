...
function printHelp(): void {
  process.stdout.write(
    [
      ...
      '',
      'Local recovery mode for this restored source tree.',
      '',
      'Options:',
      '  -h, --help                    Show help',
...
function printVersion(): void {
  process.stdout.write('2.1.88-local (Claude Code local recovery)\n')
}
...
function parseArgs(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === '--bare') continue
    if (arg === '--dangerously-skip-permissions') continue
    if (arg === '--model') { model = argv[++i]; continue }
    if (arg === '--system-prompt') { systemPrompt = argv[++i]; continue }
    if (arg === '--system-prompt-file') {
      const file = argv[++i]
      systemPrompt = readFileSync(file!, 'utf8')
      continue
    }
    if (arg === '--append-system-prompt') { appendSystemPrompt = argv[++i]; continue }
    if (arg === '--output-format') {
      const value = argv[++i]
      if (value === 'json' || value === 'text') outputFormat = value
      continue
    }
...
async function run(): Promise<void> {
  ...
  if (parsed.command === 'version') { printVersion(); return }
...
async function runInteractive(parsed: {
  ...
  process.stdout.write(`Claude Code local interactive mode\nmodel: ${model}\ncommands: /exit, /clear\n\n`)
  rl.prompt()
  ...
  for await (const line of rl) {
    ...
    if (input === '/clear') { messages.length = 0; process.stdout.write('history cleared\n'); rl.prompt(); continue }
